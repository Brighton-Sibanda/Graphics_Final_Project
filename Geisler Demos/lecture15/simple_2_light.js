...
const FSHADER_SOURCE = `
    ...
    void main() {
        ...
        // each light gets a "base" color
        // note that this implies our teapot has no color!
...
function main() {
    // Setup our sliders
    slider_input = document.getElementById('slider1X')
    slider_input.addEventListener('input', (event) => {
        updateLight1X(event.target.value)
    })
    slider_input = document.getElementById('slider2X')
    slider_input.addEventListener('input', (event) => {
        updateLight2X(event.target.value)
    })
...
function startRendering() {
    ...
    g_u_light2_ref = gl.getUniformLocation(gl.program, 'u_Light2')
...
function draw() {
    ...
    // Update with our light positions to be behind the camera
    gl.uniform3f(g_u_light1_ref, g_light1X, 0, 2)
    gl.uniform3f(g_u_light2_ref, g_light2X, 0, 2)
...
function updateLight1X(amount) {
    label = document.getElementById('light1X')
    label.textContent = `Light 1 X: ${Number(amount).toFixed(2)}`
    g_light1X = Number(amount)
}

function updateLight2X(amount) {
    label = document.getElementById('light2X')
    label.textContent = `Light 2 X: ${Number(amount).toFixed(2)}`
    g_light2X = Number(amount)
}
