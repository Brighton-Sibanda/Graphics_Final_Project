...
async function loadGLSLFiles() {
    ...
    g_vshaderDepth = await fetch('./depth_map_visualize.vert').then(response => response.text()).then((x) => x)
    g_fshaderDepth = await fetch('./depth_map_visualize.frag').then(response => response.text()).then((x) => x)
...
function startRendering() {
    ...
    g_programDepth = createProgram(gl, g_vshaderDepth, g_fshaderDepth)
    if (!g_programDepth) {
        console.log('Failed to intialize shaders.')
        return
    }
    ...
    // reference to the texture shader pointer
    g_model_ref_depth = gl.getUniformLocation(g_programDepth, 'u_Model')
    g_texture_ref_depth = gl.getUniformLocation(g_programDepth, 'u_Texture')
...
function drawDepthMap() {
    // Helper function to visualize buffers
    gl.useProgram(g_programDepth)

    if (setupVec(3, g_programDepth, 'a_Position', 0, g_teapotMesh.length * FLOAT_SIZE) < 0) {
        return -1
    }
    if (setupVec(2, g_programDepth, 'a_TexCoord', 0, (g_teapotMesh.length + CUBE_MESH.length) * FLOAT_SIZE * 2) < 0) {
        return -1
    }
    ...
    // use our data texture
    gl.uniform1i(g_texture_ref_depth, g_dataTexture)

    // use a identity matrices except for the model (so it fills the screen)
    gl.uniformMatrix4fv(g_model_ref_depth, false, new Matrix4().scale(2, 2, -1).elements)
