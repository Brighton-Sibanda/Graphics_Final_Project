...
async function loadGLSLFiles() {
    g_vshader = await fetch('./scene_depth.vert').then(response => response.text()).then((x) => x)
    g_fshader = await fetch('./scene_depth.frag').then(response => response.text()).then((x) => x)
