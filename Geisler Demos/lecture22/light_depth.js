...
async function loadGLSLFiles() {
    g_vshader = await fetch('./light_depth.vert').then(response => response.text()).then((x) => x)
    g_fshader = await fetch('./light_depth.frag').then(response => response.text()).then((x) => x)
...
function startRendering() {
    ...
    if (setupVec(3, gl.program, 'a_Position', 0, 0) < 0) {
        return
    }
    ...
    g_light_projection_ref = gl.getUniformLocation(gl.program, 'u_LightProjection')
    ...
    // Setup an orthographic projection for the light
    g_lightProjectionMatrix = new Matrix4().setOrtho(-10, 10, -10, 10, -200, 200)
...
function draw() {
    ...
    // setup our camera and projection
    cameraMatrix = calculateCameraMatrix()
    gl.uniformMatrix4fv(g_camera_ref, false, cameraMatrix.elements)
    gl.uniformMatrix4fv(g_projection_ref, false, g_projectionMatrix.elements)

    // matrices looking from the light source
    var lightMatrix = new Matrix4().setLookAt(...g_lightPosition, 0, 0, TEAPOT_Z_CENTER, 0, 1, 0)
    gl.uniformMatrix4fv(g_light_ref, false, lightMatrix.elements)
    gl.uniformMatrix4fv(g_light_projection_ref, false, g_lightProjectionMatrix.elements)

    // Setup the teapot matrices
    gl.uniformMatrix4fv(g_model_ref, false, g_modelMatrixTeapot.elements)
    gl.uniformMatrix4fv(g_world_ref, false, g_worldMatrixTeapot.elements)
    gl.drawArrays(gl.TRIANGLES, 0, g_teapotMesh.length / 3)

    // Setup the floor matrices
    gl.uniformMatrix4fv(g_model_ref, false, g_modelMatrixFloor.elements)
    gl.uniformMatrix4fv(g_world_ref, false, g_worldMatrixFloor.elements)
