...
async function loadImageFiles() {
    ...
    g_cubeImage.src = "./resources/brick_resized.png"
...
async function loadGLSLFiles() {
    g_vshader = await fetch('./a8.vert').then(response => response.text()).then((x) => x)
    g_fshader = await fetch('./a8.frag').then(response => response.text()).then((x) => x)
...
function startRendering() {
    ...
    // TODO: add tex coordinates (you _may_ want to give dummy tex coordinates to the grid)
    var data = CUBE_MESH.concat(g_gridMesh).concat(CUBE_NORMALS).concat(g_gridMesh).concat(CUBE_TEX_MAPPING).concat(g_gridMesh)
    ...
    if (!setupVec(3, 'a_Normal', 0, FLOAT_SIZE * (CUBE_MESH.length + g_gridMesh.length))) {
        return
    }
    if (!setupVec(2, 'a_TexCoord', 0, FLOAT_SIZE * (CUBE_MESH.length + g_gridMesh.length) * 2)) {
        return
    }

    // Get references to GLSL uniforms
    // TODO: add in texture references
    ...
    // Setup our model
    g_modelMatrix = new Matrix4().rotate(20, 1, 0, 0).scale(.5, .5, .5)
...
function draw() {
    ...
    gl.uniform3fv(g_u_flatcolor_ref, [0, .7, .3])
    ...
    // draw the grid
    gl.drawArrays(gl.LINES, CUBE_MESH.length / 3, g_gridMesh.length / 3)

    // draw the light source as a white cube
    gl.uniform3fv(g_u_flatcolor_ref, [1, 1, 1])
    gl.uniformMatrix4fv(g_u_model_ref, false, new Matrix4().scale(.1, .1, .1).elements)
