...
async function loadGLSLFiles() {
    g_vshader = await fetch('./texture_brick.vert').then(response => response.text()).then((x) => x)
    g_fshader = await fetch('./texture_brick.frag').then(response => response.text()).then((x) => x)
