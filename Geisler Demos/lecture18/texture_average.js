...
async function loadImageFiles() {
    ...
    g_cubeImage.src = "resources/brick.png"
...
async function loadGLSLFiles() {
    g_vshader = await fetch('/texture_average.vert').then(response => response.text()).then((x) => x)
    g_fshader = await fetch('/texture_average.frag').then(response => response.text()).then((x) => x)
...
function startRendering() {
    ...
    g_u_texel_width = gl.getUniformLocation(gl.program, 'u_TexelWidth')
    g_u_texel_height = gl.getUniformLocation(gl.program, 'u_TexelHeight')
    ...
    // Assign width and height for our texture
    gl.uniform1f(g_u_texel_width, 1.0 / g_cubeImage.width);
    gl.uniform1f(g_u_texel_height, 1.0 / g_cubeImage.height);
...
function tick() {
    ...
    // rotate around the given axis (if rotating)
    if (g_isRotating) {
