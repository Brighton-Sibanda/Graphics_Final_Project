...
async function loadGLSLFiles() {
    g_vshader = await fetch('./texture_mipmap.vert').then(response => response.text()).then((x) => x)
    g_fshader = await fetch('./texture_mipmap.frag').then(response => response.text()).then((x) => x)
