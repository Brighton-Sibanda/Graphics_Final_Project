...
var g_u_constant_ref
var g_u_linear_ref
var g_u_quadratic_ref
var g_u_spotlightSize_ref
var g_u_spotlightDirection_ref
...
function main() {
    ...
    slider_input = document.getElementById('sliderConstant')
    slider_input.addEventListener('input', (event) => {
        updateConstant(event.target.value)
    })
    slider_input = document.getElementById('sliderLinear')
    slider_input.addEventListener('input', (event) => {
        updateLinear(event.target.value)
    })
    slider_input = document.getElementById('sliderQuadratic')
    slider_input.addEventListener('input', (event) => {
        updateQuadratic(event.target.value)
    })
    slider_input = document.getElementById('sliderSize')
    slider_input.addEventListener('input', (event) => {
        updateSpotlightSize(event.target.value)
    })
...
async function loadGLSLFiles() {
    g_vshader = await fetch('/spotlight.vert').then(response => response.text()).then((x) => x)
    g_fshader = await fetch('/spotlight.frag').then(response => response.text()).then((x) => x)
...
function startRendering() {
    ...
    g_u_constant_ref = gl.getUniformLocation(gl.program, 'u_Constant')
    g_u_linear_ref = gl.getUniformLocation(gl.program, 'u_Linear')
    g_u_quadratic_ref = gl.getUniformLocation(gl.program, 'u_Quadratic')
    g_u_spotlightSize_ref = gl.getUniformLocation(gl.program, 'u_SpotlightSize')
    g_u_spotlightDirection_ref = gl.getUniformLocation(gl.program, 'u_SpotlightDirection')
...
function draw() {
    ...
    gl.uniform1f(g_u_constant_ref, g_constant)
    gl.uniform1f(g_u_linear_ref, g_linear)
    gl.uniform1f(g_u_quadratic_ref, g_quadratic)
    gl.uniform1f(g_u_spotlightSize_ref, g_spotlightSize)

    // calculate spotlight direction to always be pointing at (0, 0, 0) for this demo
    var lightDirection = new Vector3()
    lightDirection.x = g_lightPosition[0]
    lightDirection.y = g_lightPosition[1]
    lightDirection.z = g_lightPosition[2]
    lightDirection.normalize()
    gl.uniform3fv(g_u_spotlightDirection_ref, new Float32Array([lightDirection.x, lightDirection.y, lightDirection.z]))
...
function updateConstant(amount) {
    label = document.getElementById('constant')
    label.textContent = `Attenuation constant: ${Number(amount).toFixed(1)}`
    g_constant = Number(amount)
}

function updateLinear(amount) {
    label = document.getElementById('linear')
    label.textContent = `Attenuation linear: ${Number(amount).toFixed(2)}`
    g_linear = Number(amount)
}

function updateQuadratic(amount) {
    label = document.getElementById('quadratic')
    label.textContent = `Attenuation quadratic: ${Number(amount).toFixed(3)}`
    g_quadratic = Number(amount)
}

function updateSpotlightSize(amount) {
    label = document.getElementById('size')
    label.textContent = `Spotlight size: ${Number(amount).toFixed(3)}`
    g_spotlightSize = Number(amount)
}
