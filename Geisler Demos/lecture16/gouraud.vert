...
void main() {
    ...
    // calculate everything in the vertex shader!
    // Calculate our world position
    vec3 worldPosition = vec3(u_World * u_Model * vec4(a_Position, 1.0));

    // Calculate our world space normal
    vec3 worldNormal = normalize(vec3(u_ModelWorldInverseTranspose * vec4(a_Normal, 0.0)));
    ...
    v_Color = ambientColor + diffuse * diffuseColor + specular * specularColor;
}