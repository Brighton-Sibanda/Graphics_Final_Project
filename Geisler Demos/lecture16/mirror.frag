...
// Our light direction to our current fragment position
        ...
        // next, convert our position into _camera_ space
        vec3 cameraSpacePosition = vec3(
            u_Camera * u_World * u_Model * vec4(v_Position, 1.0)
        );
        ...
        // fall-off approaches infinity for a "perfect mirror" BRDF
        float specular = pow(angle, 10000.0);

        // set a constant specular color
        vec3 specularColor = vec3(1.0, 1.0, 1.0);

        gl_FragColor = vec4(specular * specularColor, 1.0);
    }
}