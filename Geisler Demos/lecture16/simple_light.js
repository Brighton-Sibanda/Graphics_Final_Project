...
async function loadGLSLFiles() {
    g_vshader = await fetch('/simple_light.vert').then(response => response.text()).then((x) => x)
    g_fshader = await fetch('/simple_light.frag').then(response => response.text()).then((x) => x)
...
function draw() {
    ...
    var inverseTranspose = new Matrix4(g_modelMatrix)
